...
export default function Employeebalence() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8080/empcashbal")
      .then((empcashbals) => {
  ...
  const handleAddFormSubmit = (event) => {
    ...
    Axios.post("http://localhost:8080/empcashbal", newEmployeebalence)
      .then((res) => {
        if (res.status == 200) {
          // success
  ...
  const handleEditFormSubmit = (event) => {
    ...
    Axios.put("http://localhost:8080/empcashbal/id", editedEmployeebalence)
      .then((res) => {
        if (res.status == 200) {
          // success
  ...
  const handleDeleteClick = (empcashbalId) => {
    Axios.delete(`http://localhost:8080/empcashbal/${empcashbalId}`)
      .then((res) => {
        if (res.status == 200) {
          // success
          console.log("Puting data", res);
