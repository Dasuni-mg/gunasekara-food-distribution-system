...
export default function BuyerCashBalence() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8080/balence")
      .then((balences) => {
  ...
  const handleAddFormSubmit = (event) => {
    ...
    Axios.post("http://localhost:8080/balence", newBalence)
      .then((res) => {
        if (res.status == 200) {
          // success
  ...
  const handleEditFormSubmit = (event) => {
    ...
    Axios.put("http://localhost:8080/balence/id", editedBalence)
      .then((res) => {
        if (res.status == 200) {
          // success
  ...
  const handleDeleteClick = (balenceId) => {
    Axios.delete(`http://localhost:8080/balence/${balenceId}`)
      .then((res) => {
        if (res.status == 200) {
          // success
          console.log("Puting data", res);
