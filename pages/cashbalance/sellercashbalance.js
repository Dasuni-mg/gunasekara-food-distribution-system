...
export default function BuyerCashSellerbalence() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8080/sellerbalence")
      .then((sellerbalences) => {
  ...
  const handleAddFormSubmit = (event) => {
    ...
    Axios.post(
      "http://localhost:8080/sellerbalence",
      newSellerbalence
    )
      .then((res) => {
        if (res.status == 200) {
  ...
  const handleEditFormSubmit = (event) => {
    ...
    Axios.put(
      "http://localhost:8080/balence/id",
      editedSellerbalence
    )
      .then((res) => {
        if (res.status == 200) {
  ...
  const handleDeleteClick = (sellerbalenceId) => {
    Axios.delete(
      `http://localhost:8080/sellerbalence/${sellerbalenceId}`
    )
      .then((res) => {
        if (res.status == 200) {
          // success
